    ...
    return (
        <article className="rounded-2xl border border-white/10 bg-[#121826] p-4 text-white">
            <header className="flex items-center justify-between">
                <div className="text-sm text-white/70">추천 조합</div>
                {s.support.modeled && (
            ...
            <div className="mt-4 grid grid-cols-3 gap-3 text-center">
                <div>
                    <div className="text-xs text-white/60">승률(추정)</div>
                    <div className="text-lg font-semibold">
                        {(s.winRateEst * 100).toFixed(1)}%
                    </div>
                </div>
                <div>
                    <div className="text-xs text-white/60">픽률(추정)</div>
                    <div className="text-lg font-semibold">
                        {(s.pickRateEst * 100).toFixed(2)}%
                    </div>
                </div>
                <div>
                    <div className="text-xs text-white/60">평균 MMR(추정)</div>
                    <div className="text-lg font-semibold">
            ...
            <details className="mt-3 text-xs text-white/70">
                ...
                <div className="mt-2 grid grid-cols-3 gap-2">
                    ...
                    <div>
                        cluster: {(s.support.fromCluster * 100).toFixed(1)}%
                    </div>
                </div>
